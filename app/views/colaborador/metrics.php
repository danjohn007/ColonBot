<?php
$pageTitle = 'Metricas - Colaborador - ' . APP_NAME;
...
<main class="max-w-7xl mx-auto px-4 py-8 mb-24">
  <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    <div>
      <h1 class="text-2xl font-bold text-gray-900">Metricas avanzadas</h1>
      <p class="text-sm text-gray-500 mt-1">Ranking, visitas, rutas, estacionalidad y prestadores.</p>
    </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
      <p class="text-xs text-gray-500 uppercase font-semibold">Sitios publicados</p>
      <p class="text-2xl font-bold text-blue-600 mt-1"><?= count($topSites ?? []) ?></p>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
      <p class="text-xs text-gray-500 uppercase font-semibold">Rutas/tipos</p>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
      <p class="text-xs text-gray-500 uppercase font-semibold">Resenas destacadas</p>
      <p class="text-2xl font-bold text-orange-600 mt-1"><?= count($recentTopReviews ?? []) ?></p>
    </div>
  </div>
      ...
      <div class="overflow-x-auto max-h-96 overflow-y-auto">
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            <tr class="border-b last:border-0 <?= (int)$t['rn'] <= 3 ? 'bg-yellow-50' : '' ?>">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    <section class="bg-white rounded-xl shadow-sm p-6 border border-gray-100 lg:col-span-2">
      <h2 class="font-semibold text-gray-900 mb-4">Visitas por prestador de servicio</h2>
      <div class="overflow-x-auto max-h-96 overflow-y-auto">
        <table class="w-full text-sm">
          <thead>
            <tr class="text-left text-xs text-gray-500 uppercase tracking-wide border-b">
              <th class="pb-2 pr-4">Prestador</th>
              <th class="pb-2 pr-4">Hoy</th>
    ...
    <section class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      <h2 class="font-semibold text-gray-900 mb-4">Nuevos prestadores</h2>
      <div class="space-y-3 max-h-96 overflow-y-auto">
        <?php foreach (array_slice($newProviders ?? [], 0, 20) as $provider): ?>
        <div class="border border-gray-100 rounded-lg p-3">
          <p class="font-semibold text-sm text-gray-900"><?= e($provider['name']) ?></p>
          <p class="text-xs text-gray-500"><?= e($provider['category_name']) ?> - <?= e($provider['owner_name']) ?></p>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <section class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      ...
      <div class="space-y-3">
        ...
        <p class="text-sm text-gray-400 text-center py-4">Sin datos de rutas/tipo de viaje</p>
        <?php endif; ?>
      </div>
    </section>

    <section class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      <h2 class="font-semibold text-gray-900 mb-4">Sitios por tipo de visitante</h2>
      <div class="space-y-3">
        <?php foreach ($byTripType ?? [] as $tt): ?>
        <div class="flex items-center justify-between p-4 border border-gray-100 rounded-xl">
          <div>
            <p class="font-medium text-gray-900"><?= e($tripLabel($tt['trip_type'])) ?></p>
            <p class="text-xs text-gray-400"><?= (int)$tt['total_businesses'] ?> sitios asociados</p>
          </div>
          <span class="text-sm font-semibold text-blue-600"><?= (int)$tt['total_visitors'] ?> visitas</span>
        </div>
        <?php endforeach; ?>
        <?php if (empty($byTripType)): ?>
        <p class="text-sm text-gray-400 text-center py-4">Sin clasificacion por familias, pareja, adultos mayores o amigos</p>
        <?php endif; ?>
      </div>
    </section>
  </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
    <section class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
      <h2 class="font-semibold text-gray-900 mb-4">Comparativo estacional por mes y semana</h2>
